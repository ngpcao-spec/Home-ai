...
describe('C03 Google Auth Supabase', () => {
  ...
  it('reprend une session OAuth et crée automatiquement le profil customer manquant', async () => {
    ...
    const auth = createGoogleCustomerAuth(runtime, () => ({
      enabled: true,
      client: { auth: { getSession: async () => ({ data: { session: { user } }, error: null }), getUser: async () => ({ data: { user }, error: null }) } },
      profiles: {
        getById: async () => null,
  ...
  it('refuse un cache getSession sans identité confirmée par getUser', async () => {
    const cachedUser = { id: 'cached-user', email: 'cached@example.com' };
    let profileReads = 0;
    const auth = createGoogleCustomerAuth(runtime, () => ({
      enabled: true,
      client: { auth: {
        getSession: async () => ({ data: { session: { user: cachedUser } }, error: null }),
