...
describe('suivi C13', () => {
  ...
  it('rend la bottom sheet C13 avec les informations et actions attendues', () => {
    ...
    ['Thợ đang đến', 'Thời gian đến', 'Quãng đường còn lại', 'Nhắn tin', 'Bắt đầu sửa chữa'].forEach((text) => assert.match(markup, new RegExp(text)));
    assert.doesNotMatch(markup, /Số điện thoại|\+84912345678|href="tel:|Gọi điện/);
  });
  ...
  it('affiche le profil professionnel compact dès accepted avec les champs facultatifs', () => {
    ...
    for (const text of ['Provider Test Nha Trang', 'Đã xác minh', '5 · 1 đánh giá', 'Thợ điện', '8 năm',
      'Sửa chữa điện dân dụng.']) assert.match(markup, new RegExp(text.replace('+', '\\+')));
    assert.match(markup, /src="https:\/\/cdn\.test\/provider\.jpg"/);
    assert.doesNotMatch(markup, /Số điện thoại|\+84901234567|href="tel:|Gọi điện/);
  });
  ...
  it('n’invente ni badge KYC ni données facultatives absentes', () => {
    ...
    assert.doesNotMatch(markup, /Đã xác minh|Kinh nghiệm|assigned-provider-introduction|Số điện thoại|href="tel:/);
  });

  it('traduit les identifiants techniques des activités dans la fiche Client', () => {
    for (const [category, label] of Object.entries({
      electricity: 'Thợ điện', plumbing: 'Thợ sửa ống nước',
      'air-conditioning': 'Thợ điều hòa', appliances: 'Thợ sửa điện gia dụng',
    })) {
      const markup = createAssignedProviderCompactMarkup({ name: 'Provider', category, activityName: category });
      assert.match(markup, new RegExp(label));
      assert.doesNotMatch(markup, new RegExp(`>${category}<`));
    }
  });
