...
export function createTechnicianSheetMarkup(technician) {
  ...
  return `<article class="map-bottom-sheet" data-selected-technician="${technician.id}">
    <p class="sheet-eyebrow">THỢ ĐƯỢC ĐỀ XUẤT</p><h2>Đã tìm thấy thợ phù hợp</h2>
    <div class="sheet-technician"><span class="technician-avatar">${technician.initials}</span><div><h3>${technician.name}</h3>${technician.verified ? '<span class="verified-badge">✓ Đã xác minh</span>' : ''}</div><strong>⭐ ${technician.rating} <small>(${technician.reviewCount} đánh giá)</small></strong></div>
    <p class="technician-description">${technician.shortDescription}</p>
    <div class="sheet-facts"><span>Kỹ năng: ${technician.categoryLabel ?? technician.category}</span><span>${technician.completedJobs} việc đã hoàn thành</span><span>Độ tin cậy ${technician.reliabilityScore ?? 95}%</span><span>${technician.distanceKm.toFixed(1)} km đường bộ</span><span>ETA khoảng ${technician.estimatedArrivalMinutes} phút</span><span>Giá tham khảo ${formatPrice(technician.indicativePrice ?? technician.priceFrom)}đ</span></div>
