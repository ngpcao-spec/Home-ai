...
export function createProviderProfileMarkup(profile, { assigned = false } = {}) {
  ...
  return `<article class="provider-profile-card" data-provider-profile-card="${escapeHtml(profile.providerId)}" aria-labelledby="provider-profile-name">
    ...
    <dl class="provider-profile-facts">
      ...
      ${profile.experienceYears == null ? '' : `<div><dt>Kinh nghiệm</dt><dd>${profile.experienceYears} năm</dd></div>`}
      ${profile.professional ? '' : `<div><dt>Khu vực phục vụ</dt><dd>${escapeHtml(profile.serviceArea)}</dd></div>
