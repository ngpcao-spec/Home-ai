...
import { initialiseAdminSupervision } from './admin-supervision.js';
...
export async function initialiseAdminApp(root, factory = createAdminAuth) {
  let auth; let busy = false; let stopped = false; let supervision;
  const render = (state) => {
    if (stopped) return;
    supervision?.stop(); supervision = null;
  ...
  const resume = async () => {
    ...
    try {
      auth ??= factory(); const access = await auth.resume();
      if (stopped) return;
      render(access.state);
      if (access.state === 'authorized' && auth.createRepository) supervision = await initialiseAdminSupervision(root, auth.createRepository());
    }
    catch { render('error'); }
    finally { busy = false; }
  };
  ...
  await resume();
  return { stop() { stopped = true; supervision?.stop(); root.removeEventListener('click', click); root.ownerDocument?.removeEventListener('visibilitychange', visibility); } };
}
