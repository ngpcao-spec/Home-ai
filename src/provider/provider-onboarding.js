...
export async function initialiseProviderOnboarding(root,repository,{onComplete,onLogout}={}) {
  ...
  const refresh=async()=>{state=await repository.getOnboardingState();data=null;error='';await draw();};
  const draw=async()=>{
    const step=getProviderOnboardingStep(state);
    if(step==='complete'){root.innerHTML=renderProviderOnboarding(state);releasePhoto();root.removeEventListener('input',inputHandler);root.removeEventListener('click',clickHandler);await onComplete?.();return;}
  ...
  root.addEventListener('input',inputHandler);root.addEventListener('click',clickHandler);await draw();
  return{stop(){releasePhoto();root.removeEventListener('input',inputHandler);root.removeEventListener('click',clickHandler);}};
}
