...
export function renderActiveProviderMission(assignment,{busy=false,message='',navigation=null,navigationLoading=false,navigationError='',diagnosing=false,supplementParent=null}={}){
...
export function renderProviderDashboard(state, { source='mock', busy=false, message='', navigation=null, navigationLoading=false, navigationError='', diagnosing=false, supplementParent=null }={}) {
  ...
  if(assignment)return `${header}${renderActiveProviderMission(assignment,{busy,message,navigation,navigationLoading,navigationError,diagnosing,supplementParent})}${renderProviderNav('home')}`;
  return `${header}
...
export async function initialiseProviderApp(root, repositoryLoader=createProgressiveProviderAppRepository, navigationLoader=prepareProviderNavigation, auth=createProviderGoogleAuth(), heartbeatFactory=createProviderLocationHeartbeat, locationAccess={classifyError:classifyGeolocationError,getState:getLocationPermissionState,mount:mountLocationPermissionGate,request:requestCurrentPosition,geolocation:globalThis.navigator?.geolocation}) {
  ...
  const renderDashboard=async()=>{
    ...
    const priorityOffer=state.offers?.find(({id})=>id===priorityOfferId);root.innerHTML=renderProviderDashboard(state,{source:repository.source,busy,message,navigation,navigationLoading,navigationError,diagnosing,supplementParent})+(offerLayer?'':renderIncomingOffer(priorityOffer));const map=root.querySelector('[data-provider-map]');if(navigation&&map)await renderProviderNavigation(map,navigation,state.provider).catch(()=>{});
  };
