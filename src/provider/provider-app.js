...
import { mountOfferDiagnostics, readOfferDiagnostics } from './offer-diagnostics.js';
...
export function renderActiveProviderMission(assignment,{busy=false,message='',navigation=null,diagnosing=false,supplementParent=null}={}){
  ...
  return `<main class="active-mission"><div class="mission-title"><span></span><h1>Chi tiết nhiệm vụ</h1><span class="mission-status">${esc(statusLabel[assignment.status]??assignment.status)}</span></div>
    <section class="mission-map-card"><div class="provider-map" data-provider-map aria-label="Bản đồ đường đến khách hàng"></div>${navigation?`<div class="map-metrics"><div><small>Vị trí của bạn</small><strong>${navigation.providerLocation.latitude.toFixed(5)}, ${navigation.providerLocation.longitude.toFixed(5)}</strong></div><div><strong>${Number(route.distanceKm).toFixed(1)} km</strong><span>ETA ${route.durationMinutes} phút</span></div>${mapHref?`<a href="${mapHref}" target="_blank" rel="noopener" data-open-provider-map>⌘ Mở bản đồ</a>`:''}</div>`:'<div class="route-loading">Đang chuẩn bị lộ trình…</div>'}</section>
    ...
    <div class="mission-actions">${assignment.status==='accepted'?`<button data-start-travel ${busy?'disabled':''}>BẮT ĐẦU DI CHUYỂN</button>`:''}${assignment.status==='travelling'?`<button data-mark-arrived ${busy||!navigation?.arrived?'disabled':''}>TÔI ĐÃ ĐẾN</button>${!navigation?.arrived?'<small>Nút được mở khi bạn ở trong phạm vi 150 m.</small>':''}`:''}</div>${renderQuoteWorkflow(assignment,{diagnosing,busy,supplementParent})}<p class="app-message" role="status">${esc(message)}</p></main>`;
}

export function renderProviderDashboard(state, { source='mock', busy=false, message='', navigation=null, diagnosing=false, supplementParent=null }={}) {
  ...
  if(assignment)return `${header}${renderActiveProviderMission(assignment,{busy,message,navigation,diagnosing,supplementParent})}${renderProviderNav('home')}`;
  return `${header}
...
export async function initialiseProviderApp(root, repositoryLoader=createProgressiveProviderAppRepository, navigationLoader=prepareProviderNavigation, auth=createProviderGoogleAuth(), heartbeatFactory=createProviderLocationHeartbeat, locationAccess={classifyError:classifyGeolocationError,getState:getLocationPermissionState,mount:mountLocationPermissionGate,request:requestCurrentPosition,geolocation:globalThis.navigator?.geolocation}) {
  ...
  let busy=false; let message=''; let navigation=null; let diagnosing=false; let editingMissionId=null; let supplementParent=null;let confirmingAcceptance=false;
  ...
  const renderDashboard=async()=>{
    ...
    const priorityOffer=state.offers?.find(({id})=>id===priorityOfferId);root.innerHTML=renderProviderDashboard(state,{source:repository.source,busy,message,navigation,diagnosing,supplementParent})+(offerLayer?'':renderIncomingOffer(priorityOffer));const map=root.querySelector('[data-provider-map]');if(navigation&&map)await renderProviderNavigation(map,navigation,state.provider).catch(()=>{});
  };
  ...
  const draw=async()=>{
    ...
    if(diagnosing && root.querySelector('[data-quote-diagnosis]')){
      ...
      if(submit)submit.disabled=busy || !canSendQuote();
      const notice=root.querySelector('.app-message');
