import { arrivalAssessment, readArrivalLocation, confirmProviderArrival } from './provider-arrival.js';
import { createMissionChatButton, getGlobalMissionChat } from '../chat/mission-chat.js';
...
import { createProviderPushManager, renderProviderPushPrompt } from './provider-push.js';
import { initialiseProviderOnboarding } from './provider-onboarding.js';
...
export function renderActiveProviderMission(assignment,{busy=false,message='',navigation=null,navigationLoading=false,navigationError='',diagnosing=false,supplementParent=null,billing=false,testMode=false,callsEnabled=false}={}){
  ...
  return `<main class="active-mission"><div class="mission-title"><span></span><h1>Chi tiết nhiệm vụ</h1><span class="mission-status">${esc(statusLabel[assignment.status]??assignment.status)}</span></div>
    ...
    <div class="mission-actions"><div class="mission-contact-actions">${createMissionChatButton(assignment,callsEnabled)}${createMissionCallButton(assignment,'provider',callsEnabled)}</div>${assignment.status==='accepted'?`<button data-start-travel ${busy?'disabled':''}>BẮT ĐẦU DI CHUYỂN</button>`:''}${assignment.status==='travelling'?`<button data-mark-arrived ${busy?'disabled':''}>TÔI ĐÃ ĐẾN</button>${testArrivalMarkup}`:''}</div>${renderQuoteWorkflow(assignment,{diagnosing,busy,supplementParent,billing})}<p class="app-message" role="status">${esc(message)}</p></main>`;
}
...
export async function initialiseProviderApp(root, repositoryLoader=createProgressiveProviderAppRepository, navigationLoader=prepareProviderNavigation, auth=createProviderGoogleAuth(), heartbeatFactory=createProviderLocationHeartbeat, locationAccess={classifyError:classifyGeolocationError,getState:getLocationPermissionState,mount:mountLocationPermissionGate,request:requestCurrentPosition,geolocation:globalThis.navigator?.geolocation},runtimeConfig=globalThis.__HOME_AI_CONFIG__) {
  ...
  if(repository.onboardingRequired)return initialiseProviderOnboarding(root,repository,{
    onComplete:async()=>{const next=await repositoryLoader();await initialiseProviderApp(root,async()=>next,navigationLoader,auth,heartbeatFactory,locationAccess,runtimeConfig);},
    onLogout:async()=>{await auth.signOut();globalThis.location?.reload();},
  });
  let state;
  try{state=await repository.load();}catch(error){error.safeStage='DASHBOARD_LOAD';throw error;}
  const pushManager=createProviderPushManager({repository,vapidPublicKey:runtimeConfig?.VAPID_PUBLIC_KEY});
  let pushPromptHost=null;
  ...
  let pricingServices=[];let serviceArea=null;let availabilityPreferences=null;let professionalProfile=null;let pricingLoading=false;let pricingError='';let pricingMessage='';let serviceAreaMessage='';let availabilityMessage='';let professionalMessage='';let professionalPhotoFile=null;let professionalPhotoPreview='';
  ...
  const callManager=repository.source==='supabase'&&repository.callServices
    ? getGlobalCallManager({documentRef:page,role:'provider',...repository.callServices}) : null;
  const chatManager=repository.source==='supabase'&&repository.chatServices
    ? getGlobalMissionChat({documentRef:page,role:'provider',...repository.chatServices}) : null;
  let callRefresh=null;
  const syncCalls=next=>{
    chatManager?.observe({mission:next.assignment});
    void chatManager?.refresh();
    if(!callManager)return;
    if(!next.assignment){callManager.observe({mission:null});return;}
    if(['completed','cancelled','expired'].includes(next.assignment.status))callManager.observe({mission:next.assignment});
    if(callRefresh)return;
    const missionId=next.assignment.id;
    callRefresh=repository.getCallSnapshot(next.assignment).then(snapshot=>{
      if(state.assignment?.id===missionId)callManager.observe(snapshot);
    }).catch(()=>callManager.reportError('Không thể đồng bộ cuộc gọi. Vui lòng thử lại.')).finally(()=>{callRefresh=null;});
  };
  const providerTestMode=isProviderTestArrivalEnabled(runtimeConfig,state.provider);
  let latestProviderPosition=null;let renderedNavigation=null;let renderedMapElement=null;let renderedMapMissionId=null;
  let unmountMapResize=null;let mapResizeMissionId=null;let mapResizeExpanded=false;
  ...
  const renderDashboard=async()=>{
    unmountMapResize?.();unmountMapResize=null;
    if(mapResizeMissionId!==state.assignment?.id){mapResizeExpanded=false;mapResizeMissionId=state.assignment?.id??null;}
    ...
    const priorityOffer=state.offers?.find(({id})=>id===priorityOfferId);root.innerHTML=renderProviderDashboard(state,{source:repository.source,busy,message,navigation,navigationLoading,navigationError,diagnosing,supplementParent,billing:state.assignment?.id===billingMissionId,testMode:providerTestMode})+(offerLayer?'':renderIncomingOffer(priorityOffer));const map=root.querySelector('[data-provider-map]');if(navigation&&map){const card=map.closest('.mission-map-card');unmountMapResize=mountProviderMapResizeGesture({card,mapElement:map,navigation,expanded:mapResizeExpanded,onExpandedChange:value=>{mapResizeExpanded=value;}});await renderProviderNavigation(map,navigation,state.provider).catch(()=>{});navigation.map.resize?.();renderedNavigation=navigation;renderedMapElement=map;renderedMapMissionId=state.assignment?.id;}else{renderedNavigation=null;renderedMapElement=null;renderedMapMissionId=null;}
  };
  ...
  const loadProfile=async()=>{pricingLoading=true;pricingError='';await renderDashboard();try{[pricingServices,serviceArea,availabilityPreferences,professionalProfile]=await Promise.all([repository.getServices(),repository.getServiceArea(),repository.getAvailabilityPreferences(),repository.getProfessionalProfile()]);}catch(error){pricingServices=[];serviceArea=null;availabilityPreferences=null;professionalProfile=null;pricingError=error?.message??'Lỗi không xác định';}finally{pricingLoading=false;await renderDashboard();}};
  ...
  const draw=async()=>{
    ...
    if(currentView==='home'&&['accepted','travelling'].includes(state.assignment?.status)&&navigation&&navigation===renderedNavigation&&state.assignment?.id===renderedMapMissionId&&liveMap&&liveMap===renderedMapElement&&liveMap.isConnected){
      if(latestProviderPosition)updateProviderNavigationLocation(navigation,state.provider,latestProviderPosition);
      const missionStatus=root.querySelector('.mission-title .mission-status');
      if(missionStatus)missionStatus.textContent=statusLabel[state.assignment.status]??state.assignment.status;
      const actions=root.querySelector('.active-mission > .mission-actions');
      const startButton=actions?.querySelector('[data-start-travel]');
      if(state.assignment.status==='travelling'&&startButton){
        startButton.remove();
        const arrivalButton=page.createElement('button');arrivalButton.dataset.markArrived='';arrivalButton.textContent='TÔI ĐÃ ĐẾN';actions.append(arrivalButton);
        if(providerTestMode){const testButton=page.createElement('button');testButton.className='test-arrival';testButton.dataset.testProviderArrival='';testButton.textContent='TEST — Giả lập đã đến';actions.append(testButton);}
      }
      actions?.querySelectorAll('[data-start-travel],[data-mark-arrived],[data-test-provider-arrival]').forEach(button=>{button.disabled=busy;});
      ...
      const routeDistance=root.querySelector('.map-metrics div:nth-child(2) strong');
      const routeEta=root.querySelector('.map-metrics div:nth-child(2) span');
      if(routeDistance)routeDistance.textContent=`${Number(navigation.route.distanceKm).toFixed(1)} km`;
      if(routeEta)routeEta.textContent=`ETA ${navigation.route.durationMinutes} phút`;
  ...
  const heartbeat=heartbeatFactory({repository,getState:()=>state,geolocation:locationAccess.geolocation,isPageActive:()=>!page?.hidden,onPosition:position=>{latestProviderPosition=position;updateProviderNavigationLocation(navigation,state.provider,position);},onState:next=>{state=next;syncOfferLayer();},onError:async()=>{message='Không thể cập nhật GPS. Hãy cho phép truy cập vị trí.';if(currentView==='home')await draw();}});
  ...
  if(repository.source==='supabase'){
    ...
    if(page?.createElement&&page.body){pushPromptHost=page.createElement('div');pushPromptHost.dataset.providerPushHost='';page.body.append(pushPromptHost);pushPromptHost.addEventListener('click',async event=>{if(!event.target.closest('[data-enable-provider-push]'))return;showPushPrompt({status:'requesting'});showPushPrompt(await pushManager.enable().catch(()=>({status:'unavailable',message:'Không thể đăng ký thiết bị. Vui lòng thử lại.'})));});}
    void pushManager.start(showPushPrompt).catch(()=>showPushPrompt({status:'unavailable',message:'Vui lòng mở lại HOME AI và thử lại.'}));
    const openPushedOffer=async ref=>{const id=ref?await repository.resolvePushOffer(ref):await pushManager.resolveLaunch();if(!id)return;state=await repository.load();priorityOfferId=state.offers?.some(offer=>offer.id===id)?id:null;currentView='home';await draw();};
    const openPushedMessage=async ref=>{const id=ref?await repository.resolvePushMessage(ref):await pushManager.resolveMessageLaunch();if(!id)return;state=await repository.load();currentView='home';await draw();if(state.assignment?.id===id){chatManager?.observe({mission:state.assignment});chatManager?.open();}};
    void openPushedOffer().catch(()=>{});
    void openPushedMessage().catch(()=>{});
    globalThis.navigator?.serviceWorker?.addEventListener?.('message',event=>{if(event.data?.type==='mission_offer_push')void openPushedOffer(event.data.offerRef).catch(()=>{});if(event.data?.type==='mission_message_push')void openPushedMessage(event.data.messageRef).catch(()=>{});});
  }
  ...
  const handleProviderClick=async e=>{
    ...
    if(e.target.closest('[data-save-service-area]')){
      ...
      if(!draft.valid){serviceAreaMessage='Vui lòng chọn bán kính hợp lệ.';await draw();return;}
      busy=true;serviceAreaMessage='';await draw();
      try{serviceArea=await repository.setServiceArea(draft.serviceRadiusKm);serviceAreaMessage='Đã lưu khu vực hoạt động.';}
      catch(error){serviceAreaMessage=error?.message??'Không thể lưu khu vực hoạt động.';}
      finally{busy=false;await draw();}
      return;
    }
