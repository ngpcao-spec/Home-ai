...
export function renderProviderKyc(state,{stage='capture',previewUrl='',error='',busy=false,testMode=false,showBack=false,assistMode=false}={}){
  ...
  if(!assistMode&&state?.submission?.status==='pending_review')return `${header}<main class="provider-kyc provider-kyc-status"><span>✓</span><p>XÁC MINH DANH TÍNH</p><h1>Hồ sơ của bạn đang được xác minh.</h1><small>HOME AI sẽ thông báo khi quá trình xem xét hoàn tất.</small></main>`;
  ...
  if(stage==='confirm'&&submission){const fields=submission.confirmedFields??Object.fromEntries(Object.entries(submission.extraction?.fields??{}).map(([key,field])=>[key,field?.value]));return `${header}<main class="provider-kyc">${testMode?'<div class="kyc-test-banner">TEST · Không lưu tài liệu hoặc hồ sơ xác minh</div>':''}<p class="pricing-kicker">${assistMode?'ĐIỀN HỒ SƠ NHANH':'XÁC MINH DANH TÍNH'}</p><h1>Xác nhận thông tin</h1><p>${assistMode?'Kiểm tra và chỉnh sửa thông tin trước khi tiếp tục. HOME AI chưa xác minh danh tính của bạn.':'Kiểm tra và sửa thông tin trước khi gửi HOME AI xem xét.'}</p>${previewUrl?`<img class="kyc-preview" src="${esc(previewUrl)}" alt="Mặt trước CCCD đã chọn">`:''}<form data-provider-kyc-form>${FIELDS.map(([name,label])=>`<label>${label}${submission.extraction?.fields?.[name]?.confidence<.65?'<small>Độ tin cậy thấp — vui lòng kiểm tra</small>':''}<input name="${name}" value="${esc(fields?.[name]??'')}" ${assistMode?(name==='full_name'?'required':''):(['full_name','identity_number','date_of_birth'].includes(name)?'required':'')}></label>`).join('')}<button data-confirm-provider-kyc ${busy?'disabled':''}>${testMode?'Hoàn tất TEST':assistMode?'Tiếp tục':'Xác nhận thông tin'}</button></form><p class="app-message" role="alert">${esc(error)}</p></main>`;}
  return `${header}<main class="provider-kyc">${testMode?'<div class="kyc-test-banner">TEST · Phân tích tạm thời, không lưu dữ liệu</div>':''}<p class="pricing-kicker">HỒ SƠ PROVIDER</p><h1>${assistMode?'Chụp CCCD để điền thông tin nhanh hơn':'Xác minh danh tính'}</h1><p>${assistMode?'HOME AI sẽ tự động đọc thông tin. Bạn có thể kiểm tra và chỉnh sửa trước khi tiếp tục.':'Chụp mặt trước CCCD để HOME AI điền trước thông tin. Kết quả luôn cần bạn xác nhận và được HOME AI xem xét.'}</p><div class="kyc-card-guide" aria-hidden="true"><span>CCCD</span><div></div><i></i></div><ul><li>Chụp mặt trước CCCD</li><li>Toàn bộ thẻ phải hiển thị</li><li>Ảnh rõ nét, đủ ánh sáng</li><li>Tránh phản chiếu</li></ul><div class="kyc-actions"><label>Chụp ảnh<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif,.heic,.heif" capture="environment" data-provider-kyc-file></label><label>Chọn từ thư viện<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif,.heic,.heif" data-provider-kyc-file></label></div>${assistMode?'<button type="button" class="kyc-manual-entry" data-skip-provider-identity-assist>Nhập thông tin thủ công</button>':''}${state?.submission?.status==='rejected'&&!assistMode?'<p class="kyc-rejected">Ảnh trước đã bị từ chối. Vui lòng gửi ảnh mới rõ hơn.</p>':''}<p class="app-message" role="alert">${esc(error)}</p></main>`;
}
...
export function readProviderIdentityAssistForm(root){const form=root.querySelector('[data-provider-kyc-form]');const fields=Object.fromEntries(FIELDS.map(([name])=>[name,form?.elements?.namedItem(name)?.value?.trim()||null]));const identity=fields.identity_number??'';return Object.freeze({fields:Object.freeze(fields),valid:Boolean(fields.full_name&&(!identity||/^\d{9,12}$/.test(identity)))});}
